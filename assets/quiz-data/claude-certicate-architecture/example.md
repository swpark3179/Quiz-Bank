...
자동화된 코드 리뷰가 주석과 docstring을 분석합니다. 현재 프롬프트는 Claude에게 *"주석이 정확하고 최신 상태인지 확인하라"* 고 지시합니다. 그 결과, 허용 가능한 패턴(TODO 표시, 단순한 설명)은 자주 문제로 표시하면서, 정작 **코드가 더 이상 수행하지 않는 동작을 설명하는 주석**은 놓칩니다. 이처럼 **일관성 없는 분석의 근본 원인**을 해결하는 변경은 무엇입니까?

- **①** 오해를 부르는 주석의 few-shot 예시를 추가해, 모델이 코드베이스의 비슷한 패턴을 인식하도록 돕는다
- **②** git blame 데이터를 포함해, 최근 코드 수정보다 먼저 작성된 주석을 식별한다
- **③** 명확한 기준을 명시한다 — 주석이 주장하는 동작이 **실제 코드 동작과 모순될 때만** 플래그한다
- **④** 분석 전에 TODO·FIXME·서술형 주석 패턴을 걸러내 노이즈를 줄인다
...
파이프라인 스크립트가 `claude "Analyze this pull request for security issues"` 를 실행하는데 작업이 무한정 멈춰 있습니다. 로그를 보면 Claude Code가 **대화형 입력을 기다리는** 중입니다. 자동화 파이프라인에서 Claude Code를 실행하는 올바른 방법은 무엇입니까?

- **①** `-p` 플래그를 추가한다 — `claude -p "Analyze this pull request for security issues"`
- **②** stdin을 `/dev/null` 에서 리다이렉트한다 — `claude "..." < /dev/null`
- **③** 명령 실행 전에 환경 변수 `CLAUDE_HEADLESS=true` 를 설정한다
- **④** `--batch` 플래그를 추가한다 — `claude --batch "..."`
